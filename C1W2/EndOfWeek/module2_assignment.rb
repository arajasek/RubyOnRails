...
class LineAnalyzer
  ...
  def calculate_word_frequency
    ...
    word_frequency.each do |k,v|
      @highest_wf_words.push(k) if v == @highest_wf_count
    end
  end

  def pretty_print
    print @highest_wf_words
    puts " (appears in line #{@line_number})"
  end
...
class Solution
  ...
  def analyze_file
    if File.exist? 'test.txt'
      lno = 0
      File.foreach('test.txt') do |ln|
        @analyzers.push(LineAnalyzer.new(ln, lno))
        lno += 1
      end
    end
  end 
  ...
  def print_highest_word_frequency_across_lines
    ...
    @highest_count_words_across_lines.each do |anal|
      anal.pretty_print
    end    
  end
end
